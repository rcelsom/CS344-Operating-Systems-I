...
void read_socket(char* key, int establishedConnectionFD){
	...
	//	printf("SERVER: this is the recieve buffer: %s\n", buffer);
	strcpy(key, buffer);
}
...
/*************************************************
 * Function: 		mod()          
 * Preconditions:	The decrypt() function is called.     	
 * Postconditions:    	This function will perform the mod operation, and ensure that it returns the proper value if the number is negative.
 * Description:       	This function find the remainder of a number. If the result is negative. Then then the remainder +27 is returned.
 * Return:           	an integer. 
 ***********************************************/
int mod(int a){
	int r = a % 27;
 ...
 * Postconditions:      The function encrypts the ciphertext into plaintext.
 * Description:  	This function decrypt cipher text using a key. First all spaces are converted to the value 91. Then the plaintext value is subtracted
 * 			from the value of the key. the reuslt is the passed to the mod() function. The result is then added to 65. This is the plaintext result. 
 * 			This process is iterated a number of times equal to the length of the plaintext.  
 * Return:              N/A
 ***********************************************/
void decrypt(char* key, char* plaintext){
	...
	for(i = 0; i < strlen(plaintext); i++){
		...
		int temp1 =plaintext[i];
		int temp2 =key[i];

		char num = temp1-temp2;
		num = mod(num) + 65;	
		if(key[i] == 91)
			key[i] = 32;
 ...
 * Postconditions:      The buffer is split at the "@" character and the key and ciphertext c-string.
